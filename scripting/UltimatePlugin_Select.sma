...
*/
#define ADMIN_LISTEN_ACCESS		ADMIN_BAN
...
new g_iPluginStatus[ PLUGINS_MAX ];
...
public plugin_init( ) {
	...
	/* Client Commands */ 
	if( g_iPluginStatus[ ADMIN_GAG ] || g_iPluginStatus[ ADMIN_COLOR_CHAT ] || g_iPluginStatus[ SMART_SLASH ] ) {
		register_clcmd( "say",			"ClCmd_SayHandler" );
		register_clcmd( "say_team",		"ClCmd_SayTeamHandler" );
	}
	
	if( g_iPluginStatus[ RESET_SCORE ] ) {
	...
	if( g_iPluginStatus[ ADMIN_CHECK ] ) {
	...
	if( g_iPluginStatus[ TO_SPEC_AND_BACK ] )  {
	...
	if( g_iPluginStatus[ ADMIN_ARMOR ] ) {
	...
	if( g_iPluginStatus[ ADMIN_COLOR_CHAT ] ) {
	...
	/* Ham Hooks */
	if( g_iPluginStatus[ SPAWN_PROTECTION ] ) {
		RegisterHam( Ham_TakeDamage,	"player",		"Ham_TakeDamage_Player_Pre",		false );
	}
	if( g_iPluginStatus[ ADMIN_BADAIM ] ) {
		RegisterHam( Ham_Player_PreThink,	"player",	"Ham_Player_PreThink_Pre",	false );
	}
	RegisterHam( Ham_Spawn,		"player",		"Ham_Spawn_Player_Post",		true );
	RegisterHam( Ham_Killed,	"player",		"Ham_Killed_Player_Post",		true );

	/* Events */ 
	if( g_iPluginStatus[ ADMIN_SPEED ] || g_iPluginStatus[ ADMIN_AMMO ] ) {
		register_event( "CurWeapon",		"Event_CurWeapon",		"be", "1=1" );
	}
	if( g_iPluginStatus[ ADVANCED_BULLET_DAMAGE ] ) {
		register_event( "Damage",		"Event_Damage",			"b", "2!0", "3=0", "4!0" );
	}
	if( g_iPluginStatus[ ADMIN_LISTEN ] || g_iPluginStatus[ DEAD_LISTEN ] ) {
	...
	if( g_iPluginStatus[ AFK_BOMB_MANAGER ] ) {
	...
	/* Forwards */
	if( g_iPluginStatus[ MUTE_MENU ] || g_iPluginStatus[ ADMIN_GAG ] ) {
	...
	if( g_iPluginStatus[ ADMIN_NOBUY ] || g_iPluginStatus[ AUTO_RESTART ] ) {
		register_message( get_user_msgid( "StatusIcon" ),	"Message_StatusIcon" );
	}
	
	/* Tasks */
	if( g_iPluginStatus[ AFK_MANAGER ] ) {
...
public client_authorized( iPlayerID ) {
	...
	if( g_iPluginStatus[ AFK_BOMB_MANAGER ] && !task_exists( TASK_AFK_BOMBCHECK ) ) {
		set_task( float( AFK_FREQUENCY ), "Task_AFK_BombCheck",	TASK_AFK_BOMBCHECK,	_, _, "b" );
	}
}

public client_putinserver( iPlayerID ) {
	if( g_iPluginStatus[ JOIN_LEAVE_ANNOUNCEMENTS ] || !g_iJoinleaveAnnouncements ) {
		return PLUGIN_CONTINUE;
	}
...
public ClCmd_SayHandler( iPlayerID ) {
	...
	if( g_iPluginStatus[ SMART_SLASH ] &&  strTemp[ 0 ] == '/' ) {
		if( strTemp[ 1 ] == '^0' ) {
			if( g_iCommandCount ) {
...
public ClCmd_SayTeamHandler( iPlayerID ) {
	...
	#if defined GREEN_CHAT
	if( g_iPluginStatus[ ADMIN_COLOR_CHAT ] && strTemp[ 0 ] == '@' ) {
		new strMessage[ 192 ];
		read_args( strMessage, 191 );
		remove_quotes( strMessage );
...
public Ham_Spawn_Player_Post( iPlayerID ) {
	...
	if( g_iPluginStatus[ ADMIN_NOCLIP ] && CheckBit( g_bitHasNoClip, iPlayerID ) ) {
		set_user_noclip( iPlayerID, 1 );
	}
	
	if( g_iPluginStatus[ AUTO_RESTART ] && CheckBit( g_bitCvarStatus, CVAR_AUTORR_KNIFE ) && task_exists( TASK_COUNTDOWN_RESTARTROUND ) ) {
		StripPlayerWeapons( iPlayerID );
	}

	if( g_iPluginStatus[ ADMIN_GODMODE ] && CheckBit( g_bitHasGodmode, iPlayerID ) ) {
		set_user_godmode( iPlayerID, 1 );
	}
	
	if( g_iPluginStatus[ SPAWN_REFUND ] && CheckBit( g_bitCvarStatus, CVAR_REFUND ) ) {
	...
	if( g_iPluginStatus[ ADMIN_AUTOSLAY ] && CheckBit( g_bitHasAutoSlay, iPlayerID ) ) {
		set_task( DELAY_AUTOSLAY, "Task_AutoSlayPlayer", TASK_AUTOSLAY + iPlayerID );
	}
	
	if( g_iPluginStatus[ SPAWN_PROTECTION ] && g_iSpawnProtection ) {
...
public Ham_Killed_Player_Post( iPlayerID ) {
	...
	if( g_iPluginStatus[ SPEC_MENU_BUG_FIX ] ) {
...
public Event_CurWeapon( iPlayerID ) {
	...
	if( g_iPluginStatus[ ADMIN_SPEED ] && CheckBit( g_bitHasSpeed, iPlayerID ) ) {
	...
	if( iSender == iReceiver ) {
		...
		for( iLoop = 0; iLoop < iNum; iLoop ++ ) {
			...
			if( !bCount[ iSender ][ iTempID ] ) {
				if( CheckBit( g_bitCvarStatus, CVAR_ADMINLISTEN ) && ( get_user_flags( iTempID ) & ADMIN_LISTEN_ACCESS ) ) {
...
public Event_SayText_Team( iReceiver ) {
	...
	if( iSender == iReceiver ) {
		...
		for( iLoop = 0; iLoop < iNum; iLoop ++ ) {
			...
			if( !bCount[ iSender ][ iTempID ] ) {
				if( CheckBit( g_bitCvarStatus, CVAR_ADMINLISTEN_TEAM ) && ( get_user_flags( iTempID ) & ADMIN_LISTEN_ACCESS ) ) {
...
public LogEvent_RoundStart( ) {
	if( g_iPluginStatus[ AUTO_RESTART ] && CheckBit( g_bitCvarStatus, CVAR_AUTORR ) && !g_bRestartedRound ) {
	...
	if( g_iPluginStatus[ ADMIN_SPEED ] || g_iPluginStatus[ AFK_MANAGER ] ) {
...
public API_SetUserUnAmmo( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_AMMO ] ) {
		return 0;
	}
...
public API_SetUserSpeed( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_SPEED ] ) {
		return 0;
	}
...
public API_SetUserBlanks( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_BLANKS ] ) {
		return 0;
	}
...
public API_SetUserNoBuy( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_NOBUY ] ) {
		return 0;
	}
...
public API_SetUserFire( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_FIRE ] ) {
		return 0;
	}
...
public API_SetUserAutoSlay( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_AUTOSLAY ] ) {
		return 0;
	}
...
public API_SetUserBadAim( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_BADAIM ] ) {
		return 0;
	}
...
public API_SetUserScore( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_SCORE ] ) {
		return 0;
	}
...
public API_SetUserGag( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || !g_iPluginStatus[ ADMIN_GAG ] ) {
		return 0;
	}
...
public API_RevivePlayer( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsConnected, iPlayerID ) || CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_REVIVE ] ) {
		return 0;
	}
...
public API_DrugPlayer( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_DRUG ] ) {
		return 0;
	}
...
public API_BuryPlayer( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_BURY ] ) {
		return 0;
	}
...
public API_DisarmPlayer( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_DISARM ] ) {
		return 0;
	}
...
public API_UberSlapPlayer( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_UBERSLAP ] ) {
		return 0;
	}
...
public API_RocketPlayer( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_ROCKET ] ) {
		return 0;
	}
...
public API_Slay2Player( iPlugin, iParams ) {
	...
	if( !CheckBit( g_bitIsAlive, iPlayerID ) || !g_iPluginStatus[ ADMIN_SLAY2 ] ) {
		return 0;
	}
...
ReadPluginStatus( ) {
	new strConfigDir[ 128 ];
	get_localinfo( "amxx_configsdir", strConfigDir, 127 );
	format( strConfigDir, 127, "%s/UltimatePlugin.ini", strConfigDir );
	...
	if( file_exists( strConfigDir ) ) {
		...
		arrayset( g_iPluginStatus, 1, PLUGINS_MAX );
		
		log_amx( "%s.ini file not found, all plugins enabled by default.", g_strPluginName );
	}
}
